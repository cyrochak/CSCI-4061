...
void rebuilt(char *root, char *TargetName, target_t targets[], int nTargetCount)
{
	...
	if (TargetIndex!= -1)
	{
		// printf("%d ", targets[TargetIndex].DependencyCount);
		...
		if(targets[TargetIndex].DependencyCount != 0)
		{
			for (int t = 0; t < targets[TargetIndex].DependencyCount; t++)
			{
				...
				switch(check)
				{
					case -1 :
					{
						int depend_name_index = find_target(targets[TargetIndex].DependencyNames[t], targets, nTargetCount);
						...
						if(depend_name_index == -1)
						{
							if(does_file_exist(targets[TargetIndex].DependencyNames[t])==-1)
							{
								...
								sprintf(error_massage, "./make4061: *** No rule to make target '%s'.  Stop.", targets[TargetIndex].DependencyNames[t]);
								printf("%s\n", error_massage);
								exit(0);
							}
						}
	...
	{
		char error_massage[1024];
		sprintf(error_massage, "./make4061: *** No rule to make target '%s'.  Stop.", TargetName);
		printf("%s\n", error_massage);
		exit(0);
	}
...
void show_targets(target_t targets[], int nTargetCount)
{
	...
	{
		printf("TargetName: %s DependencyCount: %d ", targets[i].TargetName,
		...
		printf("DependencyNames: %s Command: %s ",DepNames,
			targets[i].Command);
	}
}
